document.addEventListener('DOMContentLoaded', () => {
    ...
    function initLogoAnim() {
        ...
        // Misura tramite clone invisibile per evitare flash visivi ("salti" del logo)
        const clone = mainLogo.cloneNode(true);
        clone.style.transform = 'none';
        clone.style.visibility = 'hidden';
        clone.style.transition = 'none';
        mainLogo.parentNode.insertBefore(clone, mainLogo);
        ...
        requestAnimationFrame(() => {
            ...
            clone.remove(); // Pulizia del clone
            lastRenderedScrollY = -1; // Forza il re-render immediato
        });
    }
    ...
    function renderLoop() {
        ...
        if (scrolled !== lastRenderedScrollY) {
            ...
            if (mainLogo && !document.body.classList.contains('page-inner')) {
                ...
                // Da Oro (226, 182, 89) a Bianco (240, 240, 240)
...
window.handleParallax = function(e) {
    ...
    if (img) {
        img.style.transform = `translate(${x * -10}px, ${y * -7}px) scale(1.08)`;
        img.style.transition = 'transform 0.1s ease-out';
    }
};

window.resetParallax = function(e) {
    const card = e.currentTarget;
...
window.toggleIngredients = function(e, currentItem) {
    const wasOpen = currentItem.classList.contains('show-ingredients');
    ...
    // Apri solo se prima era chiusa
    if (!wasOpen) {
        currentItem.classList.add('show-ingredients');
    }
};
        ...
        if (grid) {
            ...
            items.forEach((item, i) => {
                gsap.from(item, {
                    ...
                    scale: 0.95, // Utilizza scale invece di y per evitare conflitti con il parallax
                    opacity: 0,
                    duration: 0.8,
                    ease: "power3.out"
                });
            ...
            if (window.innerWidth > 768) {
                ...
                gsap.to(leftColItems, {
                    ...
                    scrollTrigger: {
                        ...
                        scrub: 1.5 // Aumenta il tempo di scrub per creare un effetto smussato fluidissimo (Inertia)
                    }
                });
                ...
                gsap.to(rightColItems, {
                    ...
                    scrollTrigger: {
                        ...
                        scrub: 1.5 // Aumenta il tempo di scrub
                    }
                });
            }
        }
